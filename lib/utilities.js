...
var packageInfo = _interopRequireWildcard(require("../package.json"));
...
const self = module.exports = {
  ...
  removeUnsupportedCharacters(fileName) {
    return fileName.replace(new RegExp(this.unsupportedCharacters[process.platform], "g"), "");
  },
  ...
  async getBookPath(localDb, doc) {
    ...
    if (doc.parentBookId) {
      ...
      while (hasParent) {
        var parentBookData = await localDb.books.get(parentBookData ? parentBookData.parentBookId : doc.parentBookId);
        const supportedParentBookName = self.removeUnsupportedCharacters(parentBookData.name);
  ...
  async getDataAndWriteAllNotes(localDb, plainTextPath) {
    ...
    return new Promise(async (resolve, reject) => {
      await Promise.all(await allNotes.docs.map(async doc => {
        ...
        if (bookData) {
          let bookPath = await self.getBookPath(localDb, bookData);
          const supportedDocTitle = self.removeUnsupportedCharacters(doc.title);
          self.dataMap.books[doc.bookId] = bookPath;
          self.dataMap.notes[doc._id].path = `${bookPath}/${supportedDocTitle}.md`;
  ...
  async importAll() {
    ...
    await Promise.all(Object.keys(diskDataMap.notes).map(async noteId => {
      ...
      try {
        const currentNote = await self.localDb.notes.get(noteId); // Don't bother if there are no changes:
    ...
    const notebooks = await self.localDb.books.all({
      limit: 999999
    });
    let notebooksSupportedName = {};
    notebooks.map(notebook => {
      notebooksSupportedName[self.removeUnsupportedCharacters(notebook.name)] = notebook._id;
    });
