...
const self = module.exports = {
  ...
  async getDirectories(source) {
    const allDirectories = await _fs.promises.readdir(source, {
      withFileTypes: true
    });
    return allDirectories.filter(dirent => {
      return dirent.isDirectory();
    }).map(async dirent => {
      const subDirents = await Promise.all(await self.getDirectories(path.join(source, dirent.name)));
      return {
  ...
  async writeMaps(plainTextPath, maps) {
    maps.directories = await Promise.all(await self.getDirectories(plainTextPath));
  ...
  async importAll() {
    ...
    const allDirectories = await Promise.all(await self.getDirectories(plainTextPath));
