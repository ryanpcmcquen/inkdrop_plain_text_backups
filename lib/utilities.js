...
const self = module.exports = {
  ...
  async getTree(source) {
    ...
    return initialRead.filter(dirent => {
      return dirent.isDirectory() || // We could support more than Markdown in the future.
      dirent.isFile() && /\.md$/.test(dirent.name);
    }).map(async dirent => {
      if (dirent.isDirectory()) {
        self.currentDir += dirent.name;
        const subDirents = await Promise.all(await self.getTree(path.join(source, dirent.name)));
        return {
  ...
  async writeMaps(plainTextPath, maps) {
    ...
    console.log(self.getAllFiles(plainTextPath)); // console.log(self.treePaths);
  ...
  async importAll() {
    ...
    const tree = await Promise.all(await self.getTree(plainTextPath)); // console.log(self.treePaths);

    await Promise.all(Object.keys(diskDataMap.notes).map(async noteId => {
      const newBody = await fs.promises.readFile(`${plainTextPath}/${diskDataMap.notes[noteId].path}`, "utf-8");
