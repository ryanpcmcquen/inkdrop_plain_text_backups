  ...
  async activate() {
    if (inkdrop && !inkdrop.isMobile) {
      ...
      if (backupPath) {
        ...
        this.writeMaps(plainTextPath, this.dataMap); // Sync stuff on changes:
          ...
          try {
            ...
            switch (typeOfChange) {
              ...
              case "book":
                ...
                if (change.doc.name !== this.dataMap.books[change.id]) {
                  const oldDataMap = JSON.parse(await _fs.promises.readFile(this.getDataMapPath(plainTextPath), "utf8"));
                  debugger;
                  await _fs.promises.rename(`${plainTextPath}/${oldDataMap.books[change.id]}`, `${plainTextPath}/${this.dataMap.books[change.id]}`);
